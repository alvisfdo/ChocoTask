...
import { isSuperUser } from "../utils/index.js";
...
const protectRoute = async (req,res,next) => {
    try {
        ...
        if(token){
            ...
            const resp = await User.findById(decodedToken.userId).select("isAdmin email organization");
...
const isSuperUserRoute = async (req,res,next) => {
    const isSuperUsr = await isSuperUser(req.user.email);
    // which user requested the some data that user is admin or not we checked by req.user.isAdmin...
    if(isSuperUsr){
        next(); // if yes user is admin then go to the next middlware means whatever user trying to access that provide
    }else{
...
export { isAdminRoute, protectRoute, isSuperUserRoute};