...
export const registerUser = async (req, res) => {
    try {
        ...
        const user = await User.create({ name, email, password, isAdmin, role, title, organization }); // Creating a new user in the data base using the User model
...
export const getTeamList = async (req, res) => {
    ...
    try {
        ...
        const users = await User.find({ organization: organization }).select("name title role email isActive"); // Retrieving the list of users with selected fields
...
export const getAdminUsersList = async (req, res) => {
    // this function for the get the all users awailable in the data base
    try {
        const users = await User.find({ isAdmin: true }).select('name title role email isActive'); // Retrieving the list of admin users with selected fields
        // find all "users" because we dont specify any condition in the find() function and select the appropiate the field using the select function
...
export const updateUserProfile = async (req, res) => {
    try {
        ...
        if (user) {
            ...
            user.organization = req.user.organization || user.organization; // Updating the organization if provided
