...
export const createTask = async (req, res) => {
    try {
        const { userId, organization } = req.user; // Extracting user ID from request
...
export const duplicateTask = async (req, res) => {
    try {
        ...
        const { userId, organization } = req.user; // Extracting user ID and organization from request
        ...
        const newTask = await Task.create({
            ...task,// spread the existing task --- this need to verify is duplicate not working
...
export const postTaskActivity = async (req, res) => {
    try {
        ...
        const task = await Task.findById(id).populate({
            path: "team",
            select: "name title role email organization",
        }); // Ensuring the task is populated with relevant user and organization details
        
        // Verify if the task belongs to the same organization
        if (task.organization.toString() !== organization.toString()) {
...
export const dashboardStatistics = async (req, res) => {
    ...
    try {
        const { userId, isAdmin, organization } = req.user; // Extracting user details from request

        // Check if the user is an admin
        const allTasks = isAdmin 
        // If the user is an admin, find all tasks that are not trashed
        ? await Task.find({ isTrashed: false, organization })
        ...
        const users = await User.find({ isActive: true, organization })
...
export const getTasks = async (req, res) => {
    try {
        ...
        let query = { isTrashed: isTrashed ? true : false,
            organization: organization // Ensure the query includes the organization
         }; // so if the isTrashed = true then it find the task which has the isTrashed=true and
