...
const Login = () => {
    ...
    useEffect(() => {
        const msgFromUrl = query.get('msg');
        if (msgFromUrl) {
            setMsg(msgFromUrl);
        }
    }, [query]);
    ...
    useEffect(() => {
        if (msg) {
            if (msg === 'invalid') {
                toast.error('Invalid or expired token', { style: toastStyle });
            } else {
                toast.error('Server Error', { style: toastStyle });
            }
        }
    }, [msg]);
            ...
            <div className='w-full min-h-screen flex items-center justify-center flex-col lg:flex-row'>
                ...
                <div className='w-full md:max-w-lg 2xl:max-w-xl flex flex-col items-center justify-center gap-5 md:gap-y-10 2xl:mt-20'>
                    ...
                    <div className='w-full md:w-1/3 p-4 md:p-1 flex flex-col jusitfy-center items-center'>
                        <form onSubmit={handleSubmit(submitHandler)} className='form-container w-full md:w-[400px] flex flex-col gap-y-6 bg-white px-10 py-14'>
